...
Strategija obrade ovisi o tipu ulaza:
  ...
  - PDF (skenirani)               -> pdf2image rasterizira -> Tesseract po stranici
...
class OCRService:
    ...
    def _process_pdf(self, data: bytes, filename: str) -> str:
        ...
        with fitz.open(stream=data, filetype="pdf") as doc:
            ...
            # Premalo teksta -> dokument je sken (slike bez tekstualnog sloja).
            if total_chars < self.TEXT_PDF_THRESHOLD:
                logger.info("Skenirani PDF (%d znakova). Prebacujem na Tesseract.", total_chars)
                return self._ocr_pdf_with_tesseract(data)

            # Dovoljno teksta -> digitalni PDF, čitamo izravno bez OCR-a.
            logger.info("Digitalni PDF (%d znakova). Koristim PyMuPDF (tekst + tablice).", total_chars)
            pages_md = [self._pdf_page_to_markdown(page) for page in doc]
    ...
    def _ocr_pdf_with_tesseract(self, data: bytes) -> str:
        """
        Put za skenirane PDF-ove: svaku stranicu rasteriziramo u sliku pa nad njom
        provodimo isti slikovni OCR kao za obične slike.
        """
        try:
        ...
        # 300 DPI je dobar kompromis: dovoljno detalja za Tesseract, a da
        # rasterizacija i memorija ostanu razumni. Niži DPI ruši točnost OCR-a.
    ...
    def _pages_to_markdown(self, pages: list[str]) -> str:
        """
        Varijanta spajanja stranica za OCR put (skenirani PDF / slike), gdje na
        ulazu imamo sirovi tekst pa svaku stranicu još provučemo kroz _to_markdown.
        (Digitalni PDF koristi _join_pages jer su stranice već u Markdownu.)
        """
        if len(pages) == 1:
